...
**Dependencies:** Task 27.7 (ParallelMachineCoordinator API ready) ✅ **COMPLETE**  
**Blocks:** Task 27.9 (Failure recovery)

---

## 🚨 **CRITICAL: Constructor & Path References**

**⚠️ Type Casting for Node Flags (MOST IMPORTANT!):**
```php
// ❌ WRONG: Will NEVER detect split/merge (DB returns string!)
...
// Why? mysqli returns TINYINT(1) as "0"/"1" (string), not int
// === 1 without cast → always false → split/merge NEVER triggered
```

**⚠️ ParallelMachineCoordinator Constructor:**
```
✅ CORRECT: new ParallelMachineCoordinator($this->db)  // 1 parameter
❌ WRONG: new ParallelMachineCoordinator($this->db, $this->org)  // 2 parameters
```

**⚠️ Use Autoload (Not require_once):**
```php
✅ CORRECT: use BGERP\Dag\ParallelMachineCoordinator;
❌ WRONG: require_once __DIR__ . '/ParallelMachineCoordinator.php';
```

**⚠️ TokenLifecycleService Location:**
```
✅ Location: source/BGERP/Service/TokenLifecycleService.php
✅ Namespace: BGERP\Service
```

---

## ⚠️ **Context from Phase 1-2 + Task 27.7 (COMPLETE)**

**Phase 1 Complete (Task 27.2-27.4):**
- ✅ TokenLifecycleService extended (5 node-level methods)
- ✅ BehaviorExecutionService refactored (calls lifecycle)
- ✅ Validation matrix (13 behaviors × 3 token types)

**Phase 2 Complete (Task 27.5-27.6):**
- ✅ ComponentFlowService (BGERP\Service) - stub methods
...
- ✅ Transaction safety, FK validation, type checks

**TokenLifecycleService Current State (from Task 27.2):**
- ✅ File: `source/BGERP/Service/TokenLifecycleService.php` (NOT BGERP\Dag!)
- ✅ Has methods: startWork, pauseWork, resumeWork, completeNode, scrapTokenSimple
- ✅ completeNode() handles: normal nodes, end nodes (Phase 1)
- ⏸️ Does NOT handle: split nodes, merge nodes (Task 27.8 adds this)

**Integration Strategy:**
- Extend completeNode() to detect node type
- Delegate to ParallelMachineCoordinator for split/merge
- Keep existing logic for normal/end nodes
- Add autoload import (no require_once)
...
### 0. Add Import Statement

**File:** `source/BGERP/Service/TokenLifecycleService.php`

**Add at top (after existing use statements):**
```php
...
{
    // 1. Call coordinator to spawn components (use autoload)
...
{
    // 1. Call coordinator to check merge readiness (use autoload)
    $coordinator = new ParallelMachineCoordinator($this->db);
...
- ✅ Propagate coordinator errors (coordinator returns error arrays, not exceptions)
- ✅ Return errors gracefully: `if (!$result['ok']) return $result;`
- ⚠️ Optional try-catch (if coordinator might throw, wrap it)
- ❌ NO silent failures
- ✅ Log all operations

**Note:** ParallelMachineCoordinator returns error arrays (not throw exceptions)
```php
// Coordinator error handling:
$result = $coordinator->handleSplit($tokenId, $nodeId);
if (!$result['ok']) {
    return $result;  // Pass through error (no wrapping needed)
}

// Optional try-catch (if paranoid about unexpected exceptions):
try {
    $result = $coordinator->handleSplit($tokenId, $nodeId);
} catch (\Throwable $e) {
    error_log("[TokenLifecycle] Coordinator exception: " . $e->getMessage());
    return ['ok' => false, 'error' => 'Coordinator failed: ' . $e->getMessage()];
}
```
...
**1. Coordinator Usage Pattern:**
```php
// ✅ CORRECT (autoload + 1 parameter):
use BGERP\Dag\ParallelMachineCoordinator;

$coordinator = new ParallelMachineCoordinator($this->db);
$result = $coordinator->handleSplit($tokenId, $nodeId);

// ❌ WRONG:
$coordinator = new ParallelMachineCoordinator($this->db, $this->org);  // Too many params!
require_once __DIR__ . '/ParallelMachineCoordinator.php';  // Use autoload instead!
```

**2. Token Update Methods:**
```php
// ⚠️ Check if updateTokenStatus() exists in TokenLifecycleService:

// Option A: If method exists (from Task 27.2):
$this->updateTokenStatus($tokenId, 'waiting');  // Single field

// Option B: If method does NOT exist:
$this->updateToken($tokenId, ['status' => 'waiting']);  // Use updateToken

// For multiple fields (always use updateToken):
...
// 1. Check TokenLifecycleService for updateTokenStatus()
// 2. If exists → use it for single status updates
// 3. If not exists → use updateToken() for all updates
// 4. Implement updateToken() as dynamic field updater
```

**2. Node Type Detection:**
```php
// ⚠️ CRITICAL: Cast flags to int (DB returns string "0"/"1")
$isSplit = !empty($node['is_parallel_split']) && (int)$node['is_parallel_split'] === 1;
$isMerge = !empty($node['is_merge_node']) && (int)$node['is_merge_node'] === 1;

// ✅ CORRECT Priority Order:
if ($isSplit) {
    // Split node (highest priority)
} elseif ($isMerge) {
    // Merge node
} elseif ($this->isEndNode($nodeId)) {
    // End node
...
// Why cast? mysqli fetch_assoc() returns TINYINT(1) as "0"/"1" (string)
// Using === 1 without cast will ALWAYS fail → split/merge never detected
```

**3. updateToken() Helper:**
```php
// Dynamic UPDATE builder for flexible field updates
// Used for: status, current_node_id, metadata, etc.
// Handles type detection (int vs string)
// Safe parameter binding
```

**4. Canonical Events & Logging:**
```php
// emitEvent() method must exist in TokenLifecycleService
// If not exists: Skip or add as stub
// Events: TOKEN_SPLIT, TOKEN_MERGE
// Phase 3: May be stub (log only)

// Concrete logging examples:
error_log("[TokenLifecycle] Split node {$nodeId}: spawned " . count($result['component_tokens']) . " components");
error_log("[TokenLifecycle] Merge node {$nodeId}: waiting for " . $result['waiting_count'] . " components");
error_log("[TokenLifecycle] Merge complete: parent token {$parentTokenId} re-activated");
...
**5. Error Propagation:**
```php
// Always propagate coordinator errors:
if (!$splitResult['ok']) {
    return $splitResult;  // Pass through error response
}

// Don't wrap in new error (loses detail)
```
...
// ⚠️ Note: Test utilities required
// - createTestToken()      - creates token in database
// - fetchToken()          - fetches single token
// - fetchComponentTokens() - fetches all components for parent
// - updateToken()         - direct DB update for test setup
// These should be added as helper methods in the test class
```
...
- ❌ NO creating new spec `.md` files (✅ task results allowed)
...
**Code:**
- `source/BGERP/Service/TokenLifecycleService.php` - File to modify ⚠️ BGERP\Service!
...
## Files Modified
- `source/BGERP/Service/TokenLifecycleService.php` (+XXX lines) ⚠️ BGERP\Service!
