# DAG Task 12: Component Serial Binding (Stage 1 – Hatthasilpa Line)
...
**Data Model:**
- New table: `job_component_serial` (or similar) to store component serial bindings
- Links: (tenant/org) + job/MO + component (bom line or product) + serial id/text

**API Exposure:**
- `trace_api.php` - Extend `serial_components` to include component tokens
- `dag_token_api.php` - Add component serial data to token details
- Job details APIs - Add component serial list
...
**Search Results:**
- ✅ `trace_api.php` has `serial_components` endpoint (line 203, 560-621)
- ✅ `getComponentsForSerial()` function exists (line 2653-2707)
...
- ⚠️ Roadmap mentions component serial binding (Phase 4) but not yet implemented
- ⚠️ `TASK_DAG_5_COMPONENT_MODEL.md` exists but is PLANNED (not implemented)
- ⚠️ `trace_api.php` has structure for component queries but only uses inventory transactions
...
**Best Anchor Point:**
- **Option A:** Job header level (`job_ticket` + `job_component_serial` table)
  - Pros: Simple, job-level aggregation
  - Cons: Doesn't track per-token component relationships
  
- **Option B:** Token-level metadata (`flow_token.metadata` JSON)
  - Pros: No schema changes needed
  - Cons: Hard to query, not normalized
  
- **Option C:** New table `job_component_serial` (recommended)
  - Pros: Normalized, queryable, supports multiple components per job
  - Cons: Requires new table and migration

**Chosen Approach:** Option C - New table `job_component_serial` for Stage 1

---

## 5. Design

### 5.1 Data Model (Stage 1)
...
### 5.2 Write Path (Stage 1 - Minimal)

**Option 1: Internal API Endpoint (Recommended for Stage 1)**
- Create `source/hatthasilpa_component_api.php` (new file)
- Action: `bind_component_serial`
- Input: `job_ticket_id`, `component_code`, `component_serial`, `final_piece_serial` (optional)
- Behavior: INSERT into `job_component_serial` table
- Feature flag: `FF_HAT_COMPONENT_SERIAL_BINDING` (default: 0, disabled)

**Option 2: Extension on Existing Job Update API**
- Extend `hatthasilpa_jobs_api.php` → `update` action
- Add `component_serials` array parameter
- Behavior: INSERT/UPDATE `job_component_serial` records
- Feature flag: Same as Option 1

**Chosen Approach:** Option 1 - Internal API endpoint for Stage 1 (can be called from job creation or manual entry)

### 5.3 Read Path & API Exposure
...
- [x] Create task spec document (this file)

### Phase 2: Data Model & Storage (Stage 1)

- [ ] Design `job_component_serial` table schema
- [ ] Create migration file: `database/tenant_migrations/YYYY_MM_add_job_component_serial.php`
- [ ] Implement write path (internal API endpoint)
- [ ] Add feature flag: `FF_HAT_COMPONENT_SERIAL_BINDING`
- [ ] Document schema in this task file
...
**Next Steps:**
1. Review and approve this task spec
2. Begin Phase 2: Data Model & Storage
3. Create migration for `job_component_serial` table
4. Implement write path (internal API)
...
```json
{
  ...
  "data": {
    "job_ticket_id": 631,
    "ticket_code": "ATELIER-20251201-0631",
    "component_serials": [
      {
        "component_code": "BODY",
        "component_serial": "MA01-HAT-DIAG-20251201-00001-A7F3-X-BODY",
...
```json
{
  ...
  "data": {
    "job_ticket_id": 632,
    "ticket_code": "ATELIER-20251201-0632",
    "component_serials": null  // or [] - backward compatible
  }
}
```
...
**Next Phase:** Phase 2 - Data Model & Storage
